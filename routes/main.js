...
router.get('/', async (req, res) => {
    ...
    res.render('web', { posts: allPosts, user:req.session.user })
})

router.get('/Automobile', async (req, res) => {
    const allPosts = await postData.getPostByCategory("Automobile")
    res.render('web', { posts: allPosts, category: "Automobile", user:req.session.user })
})

router.get('/Food', async (req, res) => {
    const allPosts = await postData.getPostByCategory("Food")
    res.render('web', { posts: allPosts, category: "Food", user:req.session.user })
})

router.get('/Course', async (req, res) => {
    const allPosts = await postData.getPostByCategory("Course")
    res.render('web', { posts: allPosts, category: "Course", user:req.session.user })
})
